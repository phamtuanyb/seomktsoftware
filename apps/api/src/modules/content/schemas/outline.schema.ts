...
export const outlineSubsectionSchema = z.object({
  ...
  bullets: z.array(z.string().min(2).max(300)).min(1).max(2),
});

export const outlineSectionSchema = z.object({
  h2: z.string().min(2).max(200),
  subsections: z.array(outlineSubsectionSchema).min(0).max(1),
});
...
export const outlineSchema = z.object({
  ...
  sections: z.array(outlineSectionSchema).min(3).max(3),
});
